...
(SELECT id, "password", last_login, is_superuser, "role", username, is_staff, is_active, timestamp_created, timestamp_updated, password_changed_timestamp, reg_person_id  FROM
    dblink('dbname=cpims_dcs', 'select id, "password", last_login, is_superuser, "role", CONCAT(username, ''_01'', NULL) as username, is_staff, is_active, timestamp_created, timestamp_updated, password_changed_timestamp, reg_person_id from auth_user where reg_person_id > 1')
